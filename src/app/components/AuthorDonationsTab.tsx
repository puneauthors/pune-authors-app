    ...
    <div className="space-y-5 pb-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <button
          ...
          className="dash-btn bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 font-bold px-4 py-2 rounded-xl shadow-sm flex items-center gap-2 active:scale-95 transition-all text-xs cursor-pointer"
        >
          <Download className="w-3.5 h-3.5 text-paa-gold" /> Export Donation Report
        </button>
      </div>

      {/* 3 Beautiful Statistical Counters */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl border-none p-4 shadow-sm text-white">
          ...
          <p className="text-[10px] text-blue-100/90 mt-1">Active & legacy library involvements</p>
        </div>
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl border-none p-4 shadow-sm text-white">
          ...
          <p className="text-[10px] text-purple-100/90 mt-1">Total copies committed to collections</p>
        </div>
        <div className="bg-gradient-to-br from-emerald-500 to-teal-500 rounded-xl border-none p-4 shadow-sm text-white">
          ...
          <p className="text-[10px] text-emerald-100/90 mt-1">Total value contributed at MRP</p>
        </div>
      </div>
      ...
      <div className="space-y-3.5">
        <div className="border-b pb-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <h3 className="text-base font-bold text-paa-navy font-serif flex items-center gap-2">
            Donation Campaigns <Megaphone className="w-4 h-4 text-purple-500" />
          </h3>
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="pl-8 pr-4 py-1.5 bg-white border border-gray-200 text-xs font-semibold outline-none focus:border-paa-navy transition-colors w-full rounded-full shadow-sm text-gray-700"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3.5">
          {filteredAnnouncements.length === 0 ? (
            <div className="col-span-full bg-white rounded-xl p-8 text-center text-gray-500 border border-dashed border-gray-200">
              <Megaphone className="w-12 h-12 text-gray-300 mx-auto mb-2" />
              <p className="text-sm font-semibold">{searchTerm ? 'No campaigns match your search' : 'No Active Campaigns'}</p>
              <p className="text-xs text-gray-400 mt-0.5">{searchTerm ? 'Try adjusting your search keywords.' : 'We will notify you here when a new donation drive is launched.'}</p>
            </div>
            ...
            // Check if description has unique custom info or just generic repetition
            const hasMeaningfulDesc = ann.description && 
              !ann.description.toLowerCase().includes('this is a book donation drive') && 
            ...
            return (
              <div 
                ...
                className="rounded-2xl shadow-sm border border-paa-navy/15 overflow-hidden flex flex-col hover:shadow-md transition-all duration-200"
              >
                ...
                <div>
                  <div className="flex justify-between items-start mb-2 gap-2">
                    ...
                    {ann.visibility === 'Closed' ? (
                      ...
                      <span className="shrink-0 px-1.5 py-0.5 bg-emerald-600 text-white text-[8px] font-extrabold rounded-full uppercase tracking-wider">
                        Active
                      </span>
                    )}
                  </div>
                  ...
                  {hasMeaningfulDesc && (
                    <p className="text-gray-600 text-[10px] mb-2.5 line-clamp-2 leading-relaxed" title={ann.description}>
                      {ann.description}
                    </p>
                  )}
                  
                  <div className="space-y-1.5 mb-3 bg-white/85 backdrop-blur-xs p-2.5 rounded-xl border border-white/60 shadow-xs text-xs">
                    <div className="flex items-start gap-1.5">
                      <MapPin className="w-3.5 h-3.5 text-blue-600 shrink-0 mt-0.5" />
                      <div>
                        {ann.library?.name && !ann.title.toLowerCase().includes(ann.library.name.toLowerCase()) && (
                          <p className="text-[11px] font-bold text-paa-navy leading-tight">{ann.library.name}</p>
                        )}
                        <p className="text-[10px] font-semibold text-gray-700">
                          {[ann.library?.city, ann.library?.state].filter(Boolean).join(', ') || ann.library?.name || 'Location Not Specified'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <p className="text-[10px] text-gray-700">
                        Deadline: <span className="font-bold text-purple-800">{new Date(ann.registrationEndDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                      </p>
                    </div>
                    ...
                    {ann.feeType && ann.feeType !== 'Free' ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <p className="text-[10px] text-gray-700 font-medium">
                          Fee: <span className="font-bold text-amber-800">{ann.feeType}</span> {ann.feeAmount ? `(₹${ann.feeAmount})` : ''}
                        </p>
                      </div>
                    ) : (
                      <div className="flex items-center gap-1.5">
                        <ShieldCheck className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                        <p className="text-[10px] text-emerald-700 font-bold">Free Registration</p>
                      </div>
                    )}
                    ...
                    {ann.collectionPoint && (
                      <div className="flex items-center gap-1.5">
                        ...
                        <p className="text-[10px] text-gray-700">
                          Hub: <span className="font-bold text-indigo-800">{ann.collectionPoint}</span>
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  {isRegistering === ann.id ? (
                    <div className="border-t pt-3 space-y-3">
                      <div className="flex items-center justify-between border-b pb-1.5 mb-1.5">
                        <h5 className="font-bold text-paa-navy text-xs">Campaign Delivery & Contact Details</h5>
                        <button onClick={() => { setIsRegistering(null); setSelectedBooks([]); }} className="p-1 text-red-500 hover:bg-red-50 rounded-full cursor-pointer"><X className="w-3.5 h-3.5" /></button>
                      </div>

                      <div className="bg-gradient-to-br from-indigo-50/70 to-blue-50/40 p-3 rounded-xl border border-indigo-100 shadow-sm space-y-3 animate-in fade-in duration-200">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          <div className="space-y-0.5">
                            <span className="text-[9px] font-bold text-indigo-500 uppercase tracking-wider block">Expected Date</span>
                            <div className="text-gray-900 font-semibold text-xs flex items-center gap-1.5">
                          ...
                          <div className="space-y-0.5">
                            <span className="text-[9px] font-bold text-indigo-500 uppercase tracking-wider block">Contact Person</span>
                            <div className="text-gray-900 font-semibold text-xs flex flex-col gap-0.5">
                              <div className="flex items-center gap-1.5">
                          ...
                          <div className="space-y-0.5">
                            <span className="text-[9px] font-bold text-indigo-500 uppercase tracking-wider block">Delivery Address</span>
                            <div className="text-gray-900 font-semibold text-[11px] flex items-start gap-1.5 leading-tight">
                              <MapPin className="w-3.5 h-3.5 text-rose-500 shrink-0 mt-0.5" />
                              <span>{ann.dispatchAddress || 'N/A'}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        <h5 className="font-bold text-paa-navy text-xs">1. Select Books & Quantities</h5>
                      </div>
                      {authorBooks.length === 0 ? (
                        <p className="text-[11px] text-red-500 bg-red-50 border border-red-100 p-2.5 rounded-lg font-medium">You don't have any approved books in the catalog yet.</p>
                      ) : (
                        <div className="space-y-1.5 max-h-40 overflow-y-auto mb-3 p-2 bg-gray-50 rounded-xl border border-gray-200">
                          {authorBooks.map(book => {
                            const isSelected = selectedBooks.some(b => b.bookId === book.id);
                            return (
                              <div key={book.id} className={`flex justify-between items-center bg-white p-2 rounded-lg border transition-colors ${isSelected ? 'border-paa-navy shadow-sm' : 'border-gray-200'}`}>
                                <div className="flex items-center gap-2 min-w-0">
                                  <BookOpen className={`w-3.5 h-3.5 ${isSelected ? 'text-paa-navy' : 'text-gray-400'} shrink-0`} />
                                  <div className="flex flex-col min-w-0">
                                    <span className="text-xs font-semibold text-paa-navy truncate" title={book.title}>{book.title}</span>
                                    <span className="text-[9px] text-gray-400 font-bold">Stock: {book.stock}</span>
                                  </div>
                                </div>
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <input 
                                    ...
                                    max={book.stock}
                                    placeholder="0"
                                    className="w-14 p-1 text-xs border rounded-md text-center font-bold focus:border-paa-navy outline-none"
                                    onChange={(e) => {
                      ...
                      {calculateFee(ann) > 0 && (
                        <div className="bg-amber-50/40 border border-amber-200 rounded-xl p-3.5 space-y-3">
                          <div className="flex justify-between items-center border-b border-amber-200/50 pb-1.5">
                            <div className="text-[11px] font-bold text-amber-800 uppercase tracking-widest">2. Pay Application Fee</div>
                            <div className="text-sm font-bold text-amber-900">Total: ₹{calculateFee(ann)}</div>
                          </div>
                          ...
                          {ann.feeType === 'Pay As You Wish' && (
                            <div className="flex items-center justify-between gap-2 bg-white p-2 rounded-lg border border-amber-200">
                              <span className="text-xs font-bold text-amber-800">Enter donation amount:</span>
                              <div className="flex items-center gap-1">
                                ...
                                <input 
                                  ...
                                  className="w-18 p-1 text-xs border rounded text-right border-gray-300 focus:border-paa-navy outline-none font-bold"
                                  placeholder="0"
                                />
                              </div>
                            </div>
                          ...
                          <div className="flex flex-col items-center py-1.5 bg-white rounded-xl border border-amber-100 shadow-sm">
                            <div className="p-1 bg-white border border-gray-100 rounded-lg shadow-sm">
                              <img src={qrCode} alt="Payment QR" className="w-28 h-28 object-cover rounded" />
                            </div>
                            <p className="text-[9px] font-bold tracking-widest text-paa-navy bg-paa-gold/20 px-2.5 py-0.5 rounded-full mt-1.5 uppercase">Scan to pay via UPI</p>
                          </div>

                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                              <label className="text-[9px] font-bold text-gray-500 uppercase block mb-0.5">Transaction ID *</label>
                              <input 
                                type="text" 
                                required 
                                ...
                                className="w-full bg-white border border-gray-300 p-1.5 rounded-lg text-xs font-semibold focus:border-paa-navy outline-none"
                              />
                            </div>
                            <div>
                              <label className="text-[9px] font-bold text-gray-500 uppercase block mb-0.5">Screenshot *</label>
                              <div 
                                onClick={() => document.getElementById(`screenshot-uploader-${ann.id}`)?.click()}
                                className="border border-dashed border-gray-300 rounded-lg p-2 text-center bg-white hover:bg-gray-50 transition-colors cursor-pointer flex flex-col items-center justify-center min-h-[42px]"
                              >
                                {paymentScreenshotUrl ? (
                                  <span className="text-[10px] font-bold text-emerald-600 flex items-center gap-1">✓ Attached</span>
                                ) : (
                                  <span className="text-[10px] font-bold text-gray-400 flex items-center gap-1"><Upload className="w-3 h-3" /> Upload</span>
                                )}
                              </div>
                              <input 
                                type="file" 
                      ...
                      <div className="flex justify-end gap-2 pt-2 border-t">
                        <button onClick={() => { setIsRegistering(null); setSelectedBooks([]); setTransactionId(''); setPaymentBlob(null); setPaymentScreenshotUrl(null); }} disabled={isProcessingPayment} className="px-3 py-1.5 border rounded-lg text-xs font-bold text-gray-500 hover:bg-gray-50 cursor-pointer">Cancel</button>
                        <button 
                          onClick={() => handleRegister(ann)}
                          disabled={selectedBooks.length === 0 || isProcessingPayment}
                          className="px-4 py-1.5 bg-paa-navy text-white rounded-lg text-xs font-bold disabled:opacity-50 hover:bg-indigo-950 active:scale-95 flex items-center gap-1.5 shadow-sm transition-all cursor-pointer"
                        >
                    ...
                    <button 
                      ...
                      className="w-full py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold rounded-lg transition-all shadow-xs active:scale-95 text-center text-[11px] flex items-center justify-center gap-1 cursor-pointer"
                    >
                    ...
                    <button 
                      disabled
                      className="w-full py-1.5 bg-gray-250 border border-gray-300 text-gray-500 font-bold rounded-lg cursor-not-allowed text-center text-[11px] tracking-wide shadow-inner"
                    >
                      Campaign Closed
                    </button>
                  ) : (
                    <button 
                      ...
                      className="w-full py-1.5 bg-paa-navy hover:bg-indigo-900 border-0 text-white font-extrabold rounded-lg transition-all shadow-xs active:scale-95 text-[11px] text-center cursor-pointer"
                    >
        ...
        {filteredAnnouncements.length > 10 && (
          <div className="flex justify-center mt-4">
            <button
              ...
              className="px-5 py-2 bg-white border border-gray-200 hover:bg-gray-50 text-paa-navy font-bold rounded-full shadow-sm transition-all flex items-center gap-1.5 text-xs cursor-pointer"
            >
              {showAllCampaigns ? (
                <>Show Less <ChevronUp className="w-3.5 h-3.5" /></>
              ) : (
                <>View All Campaigns ({filteredAnnouncements.length}) <ChevronDown className="w-3.5 h-3.5" /></>
              )}
            </button>
          </div>
        )}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            <h3 className="text-base font-bold text-paa-navy font-serif">Donation History & Tracker</h3>
            <p className="text-[11px] text-gray-500 mt-0.5">Real-time status updates of your library submissions</p>
          </div>
          <div className="relative w-full sm:w-64">
            <Search className="w-3.5 h-3.5 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
            <input
