...
export function AuthorDonationsTab({ dashboardData, onRefresh }: { dashboardData?: any, onRefresh?: () => void }) {
  ...
  const sortedAnnouncements = [...announcements].filter(a => !a.isArchived).sort((a, b) => {
    ...
    if (!aClosed && bClosed) return -1;
    return new Date(b.createdAt || b.registrationEndDate).getTime() - new Date(a.createdAt || a.registrationEndDate).getTime();
  });
    ...
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {[1,2,3].map(n => <div key={n} className="h-24 bg-white border border-paa-navy/5 animate-pulse rounded-xl shadow-sm"></div>)}
        </div>
        <div className="h-80 bg-white border border-paa-navy/5 animate-pulse rounded-xl shadow-sm"></div>
      </div>
    );
  }
    ...
    <div className="space-y-4 pb-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2.5">
        ...
        <button
          ...
          className="dash-btn bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 font-bold px-3.5 py-1.5 rounded-xl shadow-sm flex items-center gap-2 active:scale-95 transition-all text-xs cursor-pointer"
        >
      ...
      <div className="space-y-3">
        <div className="border-b pb-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2.5">
          <div className="flex items-center gap-2">
            <h3 className="text-base font-bold text-paa-navy font-serif flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-bold bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-full border border-indigo-100">
              {filteredAnnouncements.filter(a => a.visibility !== 'Closed').length} Active
            </span>
          </div>
        ...
        {filteredAnnouncements.length > 10 && (
          <div className="flex justify-center mt-3">
            <button
              ...
              className="px-4 py-1.5 bg-white border border-gray-200 hover:bg-gray-50 text-paa-navy font-bold rounded-full shadow-sm transition-all flex items-center gap-1.5 text-xs cursor-pointer"
            >
              {showAllCampaigns ? (
