    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={() => setActiveMainTab('Libraries')} className={`dash-btn border hover:bg-gray-50 shadow-sm flex items-center gap-2 ${activeMainTab === 'Libraries' ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-gray-200 text-gray-700'}`}>
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 w-full">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 w-full flex flex-col overflow-hidden">
            ...
            <div className="flex-1 p-6 bg-white">
              ...
              <div id="library-master-form" className={`border rounded-xl p-5 mb-8 shadow-sm transition-all duration-500 ${editingLib ? 'border-blue-400 bg-blue-50/40 ring-4 ring-blue-500/20' : 'bg-gray-50 border-gray-200'}`}>
                <h4 className="text-sm font-bold text-paa-navy uppercase tracking-wider mb-4 border-b border-gray-200 pb-2">{editingLib ? 'Edit Library' : 'Add New Library'}</h4>
                <form key={editingLib ? editingLib.id : 'new'} onSubmit={handleSaveLibrary} className="grid grid-cols-1 md:grid-cols-4 gap-5">
                  <div className="md:col-span-2">
                    <label className="text-[10px] font-bold text-gray-600 uppercase block mb-1">Library Name *</label>
                    <input name="libName" defaultValue={editingLib?.name} required className="w-full border border-gray-300 p-2.5 rounded-lg text-sm focus:border-paa-navy focus:ring-1 focus:ring-paa-navy outline-none shadow-sm" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-gray-600 uppercase block mb-1">Type *</label>
                    <select name="type" defaultValue={editingLib?.type || 'Public Library'} required className="w-full border border-gray-300 p-2.5 rounded-lg text-sm focus:border-paa-navy focus:ring-1 focus:ring-paa-navy outline-none shadow-sm bg-white">
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-gray-600 uppercase block mb-1">Status</label>
                    <select name="status" defaultValue={editingLib?.status || 'Active'} className="w-full border border-gray-300 p-2.5 rounded-lg text-sm focus:border-paa-navy focus:ring-1 focus:ring-paa-navy outline-none shadow-sm bg-white">
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-gray-600 uppercase block mb-1">City *</label>
                    <input name="city" defaultValue={editingLib?.city} required className="w-full border border-gray-300 p-2.5 rounded-lg text-sm focus:border-paa-navy focus:ring-1 focus:ring-paa-navy outline-none shadow-sm" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-gray-600 uppercase block mb-1">State *</label>
                    <input name="state" defaultValue={editingLib?.state} required className="w-full border border-gray-300 p-2.5 rounded-lg text-sm focus:border-paa-navy focus:ring-1 focus:ring-paa-navy outline-none shadow-sm" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-gray-600 uppercase block mb-1">Contact Person *</label>
                    <input name="contactPerson" defaultValue={editingLib?.contactPerson} required className="w-full border border-gray-300 p-2.5 rounded-lg text-sm focus:border-paa-navy focus:ring-1 focus:ring-paa-navy outline-none shadow-sm" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-gray-600 uppercase block mb-1">Contact Number *</label>
                    <input name="contactNumber" defaultValue={editingLib?.contactNumber} required className="w-full border border-gray-300 p-2.5 rounded-lg text-sm focus:border-paa-navy focus:ring-1 focus:ring-paa-navy outline-none shadow-sm" />
                  </div>

                  <div className="md:col-span-4">
                    <label className="text-[10px] font-bold text-gray-600 uppercase block mb-1">Shipping Address *</label>
                    <input name="shippingAddress" defaultValue={editingLib?.shippingAddress} required className="w-full border border-gray-300 p-2.5 rounded-lg text-sm focus:border-paa-navy focus:ring-1 focus:ring-paa-navy outline-none shadow-sm" />
                  </div>

                  <div className="md:col-span-4 flex justify-end gap-3 mt-4">
                    {editingLib && <button type="button" onClick={() => setEditingLib(null)} className="px-5 py-2.5 border border-gray-300 bg-white rounded-lg text-sm font-bold text-gray-700 hover:bg-gray-50 shadow-sm transition-colors">Cancel Edit</button>}
                    <button type="submit" className="px-6 py-2.5 bg-emerald-600 text-white rounded-lg text-sm font-bold hover:bg-emerald-700 shadow-sm transition-colors flex items-center gap-2">
                      <BookOpen size={16} /> {editingLib ? 'Update Library' : 'Add Library'}
                    </button>
                  </div>
                </form>
              </div>

              {/* Excel-style Libraries Table */}
              <div className="bg-white border border-gray-300 rounded-sm overflow-hidden shadow-sm">
                <table className="w-full text-left border-collapse">
                  <thead className="bg-[#1e7b44] border-b-2 border-[#165a31]">
                    <tr>
                      <th className="p-3 !text-[13px] font-bold !text-white uppercase tracking-wider border-r border-[#165a31]/30">Library</th>
                      <th className="p-3 !text-[13px] font-bold !text-white uppercase tracking-wider border-r border-[#165a31]/30">Location</th>
                      <th className="p-3 !text-[13px] font-bold !text-white uppercase tracking-wider border-r border-[#165a31]/30">Contact</th>
                      <th className="p-3 !text-[13px] font-bold !text-white uppercase tracking-wider text-center">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    {libraries.filter(lib => !lib.isArchived).map((lib, index) => {
                      ...
                      else if (lib.type?.includes("Public")) typeColor = "bg-emerald-100 text-emerald-800 border-emerald-200";
                      ...
                      <tr key={lib.id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-[#f4fcf7]'} hover:bg-[#e6f4ea] transition-colors`}>
                        <td className="p-3 border-r border-gray-200 align-top">
                          <div className="font-bold text-sm text-gray-900 mb-1.5">{lib.name}</div>
                          <span className={`text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-sm border ${typeColor}`}>
                            {lib.type}
                          </span>
                        </td>
                        <td className="p-3 border-r border-gray-200 align-top">
                          <div className="text-sm font-medium text-gray-800">{lib.city}</div>
                          <div className="text-[11px] text-gray-500 font-bold uppercase mt-0.5">{lib.state}</div>
                          <div className="text-[11px] text-gray-600 mt-2 bg-gray-50 p-1.5 rounded border border-gray-100">
                            {lib.shippingAddress}
                          </div>
                        </td>
                        <td className="p-3 border-r border-gray-200 align-top">
                          <div className="text-sm font-bold text-gray-800">{lib.contactPerson}</div>
                          <div className="text-xs text-gray-600 font-mono mt-0.5">{lib.contactNumber}</div>
                        </td>
                        <td className="p-3 text-center align-middle">
                          <div className="flex gap-2 justify-center">
                            <button onClick={() => { setEditingLib(lib); setTimeout(() => document.getElementById('library-master-form')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50); }} className="px-3 py-1.5 bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 rounded text-xs font-bold transition-colors">Edit</button>
                            <button onClick={() => handleDeleteLibrary(lib.id)} className="px-3 py-1.5 bg-red-50 text-red-700 hover:bg-red-100 border border-red-200 rounded text-xs font-bold transition-colors">Delete</button>
                          </div>
                        </td>
                      </tr>
                    )})}
